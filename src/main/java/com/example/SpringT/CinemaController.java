...
import static com.example.SpringT.CinemaRoom.getAllseat;
import static com.example.SpringT.CinemaRoom.seats;
import static com.example.SpringT.Ticket.*;
...
public class CinemaController {
    ...
    public Ticket bookSeat(@RequestBody Seat client) {
        ...
        if ((client.getRow() >= 0 && client.getColumn() >= 0) &&
            ...
            if (client.getRow() < 5) {
                ...
                client1 = new Ticket(uuid, new Seat(client.getRow(), client.getColumn(), 10));
                booked_seat.set(client.getRow() * 9 + client.getColumn() - 10,new Seat(client.getRow(), client.getColumn(), 10));
                booked_token.set(client.getRow() * 9 + client.getColumn() - 10, uuid);
            } else {
                UUID uuid = UUID.randomUUID();
                client1 = new Ticket(uuid, new Seat(client.getRow(), client.getColumn(), 8 ));
                booked_seat.set(client.getRow() * 9 + client.getColumn() - 10, new Seat(client.getRow(), client.getColumn(), 8));
                booked_token.set(client.getRow() * 9 + client.getColumn() - 10, uuid);
            }
            return getSeat(client, client1);
        } else {
            throw new SecurityException("The number of a row or a column is out of bounds!");
        }
    }

    private Ticket getSeat(@RequestBody Seat client, Ticket client1) {
